...
//Referenced the Solution for the tests then made adjustments.
// isbn of sample book
let book_isbn;
let book_isbnTwo;
let bookList;
...
beforeEach(async () => {
  ...
  bookList.push(result.rows[0].isbn);
  ...
  book_isbnTwo = resultTwo.rows[0].isbn;
  bookList.push(resultTwo.rows[0].isbn);
});
...
describe("POST /books", async function () {
  ...
  test("Prevents creating book without required isbn", async function () {
    const response = await request(app)
        .post(`/books`)
        .send({author: "hiho"});
